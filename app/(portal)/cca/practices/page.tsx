...
interface CcaPractice {
  ...
  common_name_cosh_id: string | null
...
interface CcaPracticesResponse {
  items: CcaPractice[]
  total: number
  limit: number
  offset: number
...
function PracticesContent() {
  ...
  const [data, setData] = useState<CcaPracticesResponse>({ items: [], total: 0, limit: PAGE_SIZE, offset: 0 })
  ...
  useEffect(() => {
    ...
    api.get<CcaPracticesResponse>(`/client/${clientId}/cca/practices?${qs.toString()}`)
      .then(r => setData(r.data))
      .finally(() => setLoading(false))
  }, [clientId, cropFilter, packageFilter, timelineFilter, l1Filter, offset])

  const chips: ActiveChip[] = useMemo(() => {
    const out: ActiveChip[] = []
    if (cropFilter) {
      const friendly = data.items[0]?.crop_name_en || cropFilter
      out.push({ key: 'crop', label: `Crop: ${friendly}` })
    }
    if (packageFilter) {
      const friendly = data.items[0]?.package_name || packageFilter
      out.push({ key: 'package', label: `Package: ${friendly}` })
    }
    if (timelineFilter) {
      const friendly = data.items[0]?.timeline_name || timelineFilter
      out.push({ key: 'timeline', label: `Timeline: ${friendly}` })
    }
    if (l1Filter) {
      out.push({ key: 'l1', label: `L1: ${l1Filter}` })
    }
    return out
  }, [cropFilter, packageFilter, timelineFilter, l1Filter, data.items])
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-0.5">
          Every practice across the company&apos;s packages. Use chips to narrow down or run cross-cutting queries
          (e.g. all Pesticide L1 practices regardless of crop).
        </p>
      </div>
              ...
              {data.items.map(p => (
                <tr key={p.id} className="hover:bg-slate-50">
                  <td className="px-5 py-3.5">
                    <Link href={`/advisory/${p.package_id}`}
                  ...
                  <td className="px-5 py-3.5">
                    <Link
                      href={`/cca/timelines?crop=${encodeURIComponent(p.crop_cosh_id)}&package=${encodeURIComponent(p.package_id)}`}
                  ...
                  <td className="px-5 py-3.5">
                    <Link
                      href={`/cca/packages?crop=${encodeURIComponent(p.crop_cosh_id)}`}
                      className="text-slate-600 text-xs hover:text-green-700">
                      {p.package_name}
                    </Link>
                  </td>
                  <td className="px-5 py-3.5 text-slate-500 hidden sm:table-cell text-xs">{p.crop_name_en}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default function CcaPracticesPage() {
  return (
