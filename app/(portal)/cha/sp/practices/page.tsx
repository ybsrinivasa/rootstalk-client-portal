...
function SpPracticesContent() {
  ...
  useEffect(() => {
    ...
    api.get<Response>(`/client/${clientId}/cha-sp/practices?${qs.toString()}`)
      .then(r => setData(r.data))
      .finally(() => setLoading(false))
  }, [clientId, cropFilter, recFilter, timelineFilter, l1Filter, offset])

  const chips: ActiveChip[] = useMemo(() => {
    const out: ActiveChip[] = []
    if (cropFilter) out.push({ key: 'crop', label: `Crop: ${data.items[0]?.crop_name_en || cropFilter}` })
    if (recFilter) out.push({ key: 'rec', label: `Problem: ${data.items[0]?.specific_problem_name_en || recFilter}` })
    if (timelineFilter) out.push({ key: 'timeline', label: `Timeline: ${data.items[0]?.timeline_name || timelineFilter}` })
    if (l1Filter) out.push({ key: 'l1', label: `L1: ${l1Filter}` })
    return out
  }, [cropFilter, recFilter, timelineFilter, l1Filter, data.items])
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Specific Problem · Practices</h1>
        <p className="text-slate-500 text-sm mt-0.5">
          Every practice across the company&apos;s SP recommendations. Use chips to narrow, or filter by L1 for cross-cutting queries.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-12 text-center border border-dashed border-slate-200">
          ...
          <p className="text-slate-600 font-medium">No SP practices match the current filters.</p>
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-slate-50">
              {data.items.map(p => (
                <tr key={p.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-5 py-3.5">
                    <Link
                      href={`/cha/sp/timelines?rec=${encodeURIComponent(p.recommendation_id)}`}
                      className="text-slate-600 text-xs hover:text-green-700">
                      {p.timeline_name}
                    </Link>
                  </td>
                  <td className="px-5 py-3.5 text-slate-600 text-xs">{p.specific_problem_name_en}</td>
...
export default function ChaSpPracticesPage() {
  return (
